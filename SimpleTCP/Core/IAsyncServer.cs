...
{
    public interface IAsyncServer<TCommandType> where TCommandType : struct, Enum
    {
        Action<NetConnection> OnRemoteSocketDisconnected { get; set; }

        Action<NetConnection> OnRemoteSocketConnected { get; set; }
        ...
        void Send(NetConnection connection, IMessage<TCommandType> message);
    }
}
